...
func (c *memoServiceClient) GenerateAiTags(ctx context.Context, in *GenerateAiTagsRequest, opts ...grpc.CallOption) (*GenerateAiTagsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GenerateAiTagsResponse)
	err := c.cc.Invoke(ctx, MemoService_GenerateAiTags_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _MemoService_GenerateAiTags_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GenerateAiTagsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MemoServiceServer).GenerateAiTags(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: MemoService_GenerateAiTags_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MemoServiceServer).GenerateAiTags(ctx, req.(*GenerateAiTagsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
